...
/**
 * Generic TF-IDF cosine ranker: pick the `k` items most relevant to `query`.
 * `textOf(item)` extracts the text to rank an item on. Returns all items (capped
 * at k) when there's nothing to rank against or the set already fits. Shared by
 * the corpus AND the judgments learning DB (lib/judgments.js).
 */
export function relevanceRank(items, textOf, query, k = 8) {
  if (!items || items.length <= k) return items || [];
  const q = tokenize(query);
  if (!q.length) return items.slice(0, k);   // image-only / no query → can't rank

  const docs = items.map(it => tokenize(textOf(it)));
  const N = docs.length;
  const df = new Map();
  ...
  return items
    .map((it, i) => { const dv = vec(docs[i]); return { it, score: dot(qv, dv) / (qn * norm(dv)) }; })
    .sort((a, b) => b.score - a.score)
    .slice(0, k)
    .filter(x => x.score > 0)              // drop items with zero overlap
    .map(x => x.it);
}
...
export function selectRelevant(examples, query, k = 8) {
  return relevanceRank(examples, (ex) => ex.content, query, k);
}
