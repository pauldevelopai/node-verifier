...
import { recordPostAnalysed, recordUsage } from './corpus-writeback.js';
...
export function mountListenerRoutes(app, getHost) {
  ...
  app.post('/api/listener/analyze', wrap(async (req, host) => {
    const { postText, postUrl, pageUrl, journalistNotes, imageBase64, imageMimeType } = req.body || {};
    if (!postText && !imageBase64) return { ok: false, error: 'no_input', message: 'Provide the post text or a screenshot.' };
    ...
    await host.log.run({ op: 'listener_analyze_start', page_url: pageUrl || null, page_matched: !!page, has_image: !!imageBase64 });

    const result = await analyzeOrigin(host, { postText, postUrl, page, journalistNotes, imageBase64, imageMimeType });
    ...
    if (result.ok) {
      const stored = await addPost(host, {
        page_url: pageUrl, post_url: postUrl, post_text: postText || null, had_image: !!imageBase64,
        page_id: page?.id || null, risk_profile: result.profile, flagged_concern: journalistNotes || null,
      });
      result.stored_id = stored.id;

      // Write-back into the SHARED corpus, so the analysis exists for the rest of
      // GROUNDED and not only in this Node's Library tab. Guarded and
      // best-effort — see lib/corpus-writeback.js.
      const corpusId = await recordPostAnalysed(host, {
        postId: stored.id, postText: postText || null, postUrl, pageUrl,
        profile: result.profile, page,
      });
      if (corpusId) {
        // Kept on the post so a later judgment flips that exact record.
        await host.store.put('posts', stored.id, { ...stored, corpus_record_id: corpusId });
      }
      await recordUsage(host, { fn: 'origin_analysis' });
    }
    return result;
  }));
  ...
  app.post('/api/listener/compare', wrap(async (req, host) => {
    ...
    if (result.ok) {
      // Persist the comparison so it's part of the Node's database and can be
      // approved/disapproved (result_type 'listen_compare').
      const id = crypto.randomUUID();
      await host.store.put('comparisons', id, {
        id, compared_at: new Date().toISOString(),
        post_ids: result.post_ids || posts.map((p) => p.id),
        comparison: result.comparison,
      });
      result.comparison_id = id;
      await recordUsage(host, { fn: 'coordination_compare' });
    }
    return result;
  }));
  ...
  app.post('/api/listener/brief', wrap(async (req, host) => {
    ...
    if (result.ok) {
      ...
      await recordUsage(host, { fn: 'weekly_brief' });
    }
