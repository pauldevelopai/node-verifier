...
import { recordOriginTracked, recordUsage } from './corpus-writeback.js';
...
export async function inspectUrl(host, { url, context = {} } = {}) {
  ...
  // Write-back into the SHARED corpus — the sourcing half of the archive. The
  // `origins` store above keeps this Node working; this is what makes the
  // ACCOUNT queryable across the platform, so the same page turning up behind a
  // dozen claims becomes visible. Guarded and best-effort.
  const corpusId = await recordOriginTracked(host, {
    originId, url, parsed, risk, context: merged,
    enrichSource: enrichment?.source || null, ads,
  });
  if (corpusId) {
    const saved = await host.store?.get('origins', originId).catch(() => null);
    if (saved) await host.store?.put('origins', originId, { ...saved, corpus_record_id: corpusId }).catch(() => {});
  }
  await recordUsage(host, { fn: 'origin_tracking' });
