...
import { recordClaimChecked, recordUsage } from './corpus-writeback.js';
import { loadCorpus } from './corpus.js';
import { imageTooLarge, IMAGE_TOO_LARGE_MSG } from './json.js';
...
// still save, so an offline newsroom isn't blocked). Does NOT use host.ai (whose
// client is built once and cached), so it's accurate even when changing a key.
...
export async function postBrief(host, body) {
  const { claimText, imageBase64, imageMimeType, sourceUrl, accountOrigin } = body;
  if (!claimText && !imageBase64) {
    return { ok: false, error: 'no_input', message: 'Provide claim text, an image, or both.' };
  }
  if (imageTooLarge(imageBase64)) return { ok: false, error: 'image_too_large', message: IMAGE_TOO_LARGE_MSG };
  await host.log.run({
    op: 'verify_claim_start', has_text: !!claimText, has_image: !!imageBase64,
    has_source_url: !!sourceUrl, has_origin: !!accountOrigin,
    origin_flags: accountOrigin?.risk?.counts?.total || 0,
  });
  const result = await verifyClaim(host, { claimText, imageBase64, imageMimeType, sourceUrl, accountOrigin });
  ...
  if (result.ok) {
    ...
    await host.store.put('claims', key, {
      ...
      // Origin tracking — who/where the claim came from + the heuristic risk panel.
      account_origin: accountOrigin || null,
    });
    result.account_origin = accountOrigin || null;
    // The store key — returned so the UI can attach an approve/disapprove
    // judgment (result_type 'verify') to this exact check (lib/judgments.js).
    result.claim_id = key;

    // Write-back into the SHARED corpus. host.store above keeps this Node
    // working; this is what makes the claim exist for the rest of GROUNDED —
    // queryable, citable, exportable. Guarded and best-effort: it returns null
    // on older runtimes that have no host.corpus, and never throws into the
    // journalist's result.
    const corpusId = await recordClaimChecked(host, {
      claimId: key,
    ...
    if (corpusId) {
      // Stored next to the claim so a later judgment flips that exact record to
      // human_verified rather than having to guess which one it was.
      const saved = await host.store.get('claims', key);
      await host.store.put('claims', key, { ...saved, corpus_record_id: corpusId });
    }

    // Adoption, recorded whether or not anyone rates this. One record per
    // newsroom per function per month — see recordUsage.
    await recordUsage(host, { fn: 'claim_verification' });
  }
  return result;
}
