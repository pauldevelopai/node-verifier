...
const USER_PROMPT_TEMPLATE = ({ claimText, hasImage, sourceUrl, fetchedText, corpus, originBlock, newsroomBlock, judgmentsBlock }) => `
Claim to verify:
"""
...
export async function verifyClaim(host, { claimText, imageBase64, imageMimeType, sourceUrl, accountOrigin }) {
  ...
  // The learning DB: past checks the newsroom has approved/disapproved, the ones
  // most similar to this claim. Folded in as ground-truth corrections so the Node
  // aligns with how the newsroom has ruled before (lib/judgments.js).
  const judgments = await selectRelevantJudgments(host, claimText, 5).catch(() => []);
  ...
  const userPrompt = USER_PROMPT_TEMPLATE({
    ...
    judgmentsBlock: formatJudgmentsForPrompt(judgments),
  });
