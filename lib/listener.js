...
import { formatNewsroomForPrompt } from './verifier.js';
import { selectRelevantJudgments, formatJudgmentsForPrompt } from './judgments.js';

// The learning DB as a prompt block, ranked to `query`. Past results the newsroom
// approved/disapproved → ground-truth corrections. Empty string when none.
async function judgmentsBlock(host, query) {
  const j = await selectRelevantJudgments(host, query, 5).catch(() => []);
  return formatJudgmentsForPrompt(j);
}

// Read the shared cross-node newsroom profile (host.profile, runtime ≥ v0.14.0):
// where this newsroom publishes + who its audience is. Grounds origin analysis in
// local context (a "foreign-manufactured" call depends on what "local" is). Empty
// string when unset → behaviour unchanged.
async function newsroomBlock(host) {
  const p = host.profile ? await host.profile.get().catch(() => null) : null;
  return formatNewsroomForPrompt(p);
}

// ─── 1. Single-post origin analysis ─────────────────────────────────

const ANALYZE_SYSTEM = `You are an editorial origin-analysis assistant for a Zambian newsroom, working on the August 2026 Zambian election cycle. Your job is to help the newsroom assess where political content on Facebook is coming from — not what it says.
...
You reason from:
  ...
  (b) the Facebook Page Transparency data the newsroom have collected for the page that posted it
...
const ANALYZE_USER = ({ postText, pageBlock, journalistNotes, postUrl, hasImage }) => `
Post text (paste from the newsroom):
"""
${postText || '(no text provided — see attached screenshot)'}
"""

${hasImage ? 'A screenshot of the post was attached. Read it carefully — extract any visible text, the page name/handle, timestamps, reactions/shares, and any Facebook UI cues (e.g. "Sponsored", page badges). Factor what you see into the origin analysis.' : ''}
...

{
  ...
  "further_checks": [
    "<a specific action the newsroom should take, e.g. 'Check Meta Ad Library for spend by this page in the last 30 days'>",
    "<another>"
  ],
...
export async function analyzeOrigin(host, { postText, postUrl, page, journalistNotes, imageBase64, imageMimeType }) {
  const pageBlock = formatPageForPrompt(page);
  const userPrompt = ANALYZE_USER({ postText, pageBlock, journalistNotes, postUrl, hasImage: !!imageBase64 });
  ...
  const learned = await judgmentsBlock(host, postText || journalistNotes || '');
  content.push({ type: 'text', text: (await newsroomBlock(host)) + learned + userPrompt });
...
const COMPARE_SYSTEM = `You are an editorial coordination-analysis assistant for the newsroom. The newsroom has given you two or more posts from different (or potentially the same) Facebook pages, and want to know whether the posts show signs of common origin — coordination, shared authorship, copy-paste behaviour, or synchronised release.
...

{
  ...
  "further_checks": [
    "<specific action the newsroom should take next>"
  ],
...
export async function comparePosts(host, posts) {
  ...
  const response = await host.ai.chat(
    [{ role: 'user', content: [{ type: 'text', text: (await newsroomBlock(host)) + COMPARE_USER({ postsBlock }) }] }],
...
const BRIEF_SYSTEM = `You are an editorial intelligence analyst for the newsroom. You have access to a week's worth of posts that the newsroom have flagged and run through origin analysis. Your job is to write a short editorial brief that helps the newsroom see patterns they could not see one post at a time.
...
Watchlist (pages the newsroom are monitoring):
${watchlistBlock}
    ...
    [
      {
        ...
        content: [
          {
            type: 'text',
            text: (await newsroomBlock(host)) + BRIEF_USER({ periodStart, periodEnd, postsBlock, watchlistBlock }),
          },
        ],
      },
    ],
