...
// Facebook (and its share/login hosts) serve a login wall to unauthenticated
// fetches — never readable text. Skip them so we don't burn an 8s timeout.
const BLOCKED_HOST = /(^|\.)(facebook\.com|fb\.com|fb\.watch|instagram\.com)$/i;

export async function fetchUrlText(url, { timeoutMs = 8000, maxChars = 6000 } = {}) {
  if (!url || !/^https?:\/\//i.test(url)) return null;
  try { if (BLOCKED_HOST.test(new URL(url).hostname)) return null; } catch { return null; }
