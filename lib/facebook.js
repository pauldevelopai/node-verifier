...
export async function fetchFacebookMeta(url, { timeoutMs = 8000 } = {}) {
  ...
  try {
    ...
    // Facebook now answers logged-off page requests with 400, not 401/403.
    // Treating only 401/403 as "blocked" meant a 400 came back as
    // `blocked: false` with every field null — reading as "we looked and the
    // page is fine" when we were refused at the door. ANY non-OK status from
    // Facebook is a refusal; say so, and carry the status so it can be seen.
    if (!res.ok) return { ok: false, blocked: true, status: res.status, reason: `http_${res.status}` };
...
export function accountRiskSignals(parsed, context = {}, posts = null) {
  ...
  // ── Group transparency (from the groups scraper / journalist) ──
  if (context.group_privacy && /priv|closed|secret/i.test(context.group_privacy)) {
    push('container', 'Posted in a PRIVATE Facebook group — membership-gated, so the post can’t be viewed independently and claims circulate inside a closed audience.', 'notable');
  }
  const members = toNum(context.member_count);
  if (members != null && members > 0) {
    push('reach', `Group has ~${members.toLocaleString()} members — that is the potential reach of anything posted here.`, members >= 50000 ? 'notable' : 'minor');
  }
